...
class CheckoutController extends Controller
{
    ...
    public function bank(Request $request)
    {
        ...
        if(session()->get('shipping_cost')) {
            ...
            $ai_id = $statement[0]->Auto_increment;
// new added
             $ext = $request->file('reciept');
                $final_name = 'payment_reciept-'.$ai_id.'.'.$ext;
                 $request->file('reciept')->move(public_path('uploads'), $final_name);
// 
            $data = array();
            if(session()->get('customer_id'))
            {
            ...
            $data['order_note'] = session()->get('order_note');
            $data['txnid'] = mt_rand(000000,999999);
            $data['shipping_cost'] = session()->get('shipping_cost');
            $data['coupon_code'] = session()->get('coupon_code');
            $data['coupon_discount'] = session()->get('coupon_amount');
            ...
            $data['fee_amount'] = $fee_amount;
            $data['net_amount'] = $net_amount;
            $data['card_last4'] = 'offline';
            $data['card_exp_month'] = 'offline';
            $data['card_exp_year'] = 'offline';
            ...
            Payer Name: '.$request->firstname.'<br> 
            Bank Name: '.$request->bankname.'<br>
            ...
            $message = str_replace('[[payment_method]]', $payment_method, $message);
            $message = str_replace('[[payment_date_time]]', date('Y-m-d H:i:s'), $message);
            $message = str_replace('[[transaction_id]]', $data['txnid'], $message);
            $message = str_replace('[[shipping_cost]]', '$'.session()->get('shipping_cost'), $message);
            $message = str_replace('[[coupon_code]]', session()->get('coupon_code'), $message);
            $message = str_replace('[[coupon_discount]]', '$'.session()->get('coupon_amount'), $message);
            $message = str_replace('[[paid_amount]]', '$'.$paid_amount, $message);
            ...
            return Redirect()->to('/')->with('success', 'Payment is successful and waiting to Approval!');
    ...
    public function telebirr(Request $request)
    {
        ...
        if(session()->get('shipping_cost')) {
            ...
            // Send Email To Customer
            $payment_method = '
            Payment Method: TeleBirr<br>

            Payer Name: '.$request->payername.'<br> 
            Payment Date: '.$request->date.'<br>
            Transaction Number: '.$request->transaction_no;
    ...
    /* -----------------Chapa Payment Getway Start here-----------------*/
    ...
    public function chapa()
    {
       ...
       $response=(env('CHAPA_SECRET_KEY'));

        //if(isset($_POST['secretKey']))
       // {
           // \Chapa\Chapa::verifyTransaction(false);
            // $payment = Chapa::initializePayment($response);
             // \Chapa\Chapa::initializePayment($response);

            //$token = $_POST['secretKey'];
            $response = ([
                'name' =>$customer_name,
                'first_name' =>$customer_name,
                'last_name' =>$customer_name,
                'key' => (env('CHAPA_SECRET_KEY')),
                'phone' =>$customer_phone,
                'amount' => $final_price,
                'tx_ref' => $reference,
                'email' => $customer_email,
                'currency' => "ETB",
                'callback_url' => route('customer.payment', [$reference]),
                "title" => 'Chapa Customer Payment',
                'description' => 'Chapa Payment',
                //'source' => $token,
                'metadata' => ['order_id' => uniqid()],
                'return_url' => route('customer.chapa.success', [$reference]),
            ]);

          // dd($response);
            //$bal = \Chapa\Chapa::verifyTransaction($response->balance_transaction);
           // $bal = Chapa::initializePayment($response->balance_transaction);
            //$payment = Chapa::initializePayment($data);
            $bal = Chapa::initializePayment($response);
           // $response = Chapa::verifyTransaction($reference);
           // dd($bal);
            //$bal = Chapa::verifyTransaction($reference);
           // return Redirect('https://checkout.chapa.co/checkout/payment/'.$reference.'');
        if ($bal['status'] !== 'success') {
            // notify something went wrong
            //return Redirect()->to('/')->with('error', 'Payment is not successful!');
        //return Redirect('https://checkout.chapa.co/checkout/payment/{$reference}');
       ...
       // $bal = Chapa::verifyTransaction($reference);
        return redirect($bal['data']['checkout_url']);
            ...
            //return redirect()->back()->with('success', 'Payment is Successful!');
             return redirect()->route('customer.payment')->with('success', 'Payment is Successful!');
 ...
 public function ChapaPaymentSuccess()
    {
         return redirect()->route('customer.chapa.success')->with('success', 'Payment is Successful!');
         // return view('customer.pages.order')->with('success', 'Payment is Successful!');
    }
...
public function initialize()
    ...
    public function callback1($reference)
    {
