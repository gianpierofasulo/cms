...
class PhotoController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        $request->validate([
            'photo_name' => 'required|image|mimes:jpeg,png,jpg,gif|max:9898789872048',
    ...
    public function update(Request $request, $id)
    {
        ...
        if($request->hasFile('photo_name')) {
            $request->validate([
                'photo_name' => 'image|mimes:jpeg,png,jpg,gif|max:9898789872048',
                'photo_order' => 'numeric|min:0|max:32767'
            ]);

             if ($photo->photo_name != null) {
                $photoPath = public_path('uploads/' . $photo->photo_name);
