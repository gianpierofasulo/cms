        ...
        <div class="type-1">
            <ul>
                @foreach($categories as $row)
                    <li><a href="{{ url('category/'.$row->category_slug) }}">{{ __($row->category_name) }}</a></li>
                @endforeach
            </ul>
        </div>
    </div>
    <div class="widget">
        <h3>{{ __('Recent Posts')}}</h3><hr style="border: 3px dashed #59B300; border-radius: 5px;">
        <div class="type-2">
            <ul>
                @php $i=0 @endphp
                @foreach($blog_items_no_pagi as $row)
                    ...
                    <li>
                        ...
                        <a href="{{ url('blog/'.$row->blog_slug) }}">{{ __($row->blog_title) }}</a>
                    </li>
                @endforeach
            </ul>
        </div>
