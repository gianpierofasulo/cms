...
@section('content')
    <div class="page-banner" style="background-image: url({{ asset('public/uploads/'.$g_setting->banner_contact) }})">
        ...
        <div class="text">
            <h1>{{ __($contact->name) }}</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb justify-content-center">
                    <li class="breadcrumb-item"><a href="{{ url('/') }}">{{__('Home')}}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ __($contact->name) }}</li>
                </ol>
            </nav>
        </div>
    </div>
    ...
    <div class="page-content">
        <div class="container">
            ...
            <div class="row contact-form">
                <div class="col-md-12">
                    <h4 class="contact-form-title mt_50 mb_20">{{__('Contact Form')}}</h4>
                    <form action="{{ route('front.contact_form') }}" method="post">
                        @csrf
                        <div class="row">
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>{{__('Name (Required)')}}</label>
                                    <input type="text" class="form-control" name="visitor_name">
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div class="form-group">
                                    <label>{{__('Email Address (Required)')}}</label>
                        ...
                        <div class="form-group">
                            <label>{{__('Message (Required)')}}</label>
                        ...
                        <button type="submit" class="btn btn-primary mt_10">{{__('Send Message')}}</button>
                    </form>
                </div>
            </div>
        </div>
