...
@section('content')
    <div class="page-banner" style="background-image: url({{ asset('public/uploads/'.$g_setting->banner_blog_detail) }})">
        ...
        <div class="text">
            <h1>{{ __($blog_detail->blog_title) }}</h1>
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb justify-content-center">
                    <li class="breadcrumb-item"><a href="{{ url('/') }}">{{ __('Home')}}</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('front.blogs') }}">{{__('Blogs')}}</a></li>
                    <li class="breadcrumb-item active" aria-current="page">{{ __($blog_detail->blog_title) }}</li>
                </ol>
            </nav>
        </div>
    </div>
            ...
            <div class="row">
                <div class="col-md-8">
                    <div class="single-section">
                        ...
                        <div class="text">
                            <h2>{{ __($blog_detail->blog_title) }}</h2>
                            <h3>
                                Posted on: {{ \Carbon\Carbon::parse($blog_detail->created_at)->format('d M, Y') }}
                            </h3>
                            {!!  __($blog_detail->blog_content) !!}
                        </div>

                        <h2 class="mt_35">{{__('Share This')}}</h2>
                        <div class="share_buttons">
                            ...
                            <a class="twitter" target="_blank" href="https://t.me/share/url?url={{ url('blog/'.$blog_detail->blog_slug) }}"><i class="fa fa-paper-plane"></i></a>
                        </div>
                        ...
                        <div class="comment mt_50">
                            ...
                            <h2 class="mb_40">{{__('Comments')}} ({{ count($comments) }})</h2>

                            @if(count($comments) == 0)
                                <div class="text-danger">{{__('No Comment Found!')}}</div>
                            @else
                            ...
                            <h2 class="mt_35">{{__('Post Your Comment')}}</h2>
